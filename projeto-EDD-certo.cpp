...
void t1_func(void * arg) {
    ...
    uart.println(buff);
    // alarme.agenda(&t1);
    timer.delay(1000000); // e1 = 1 segundo
...
void t3_func(void * arg) {
    ...
    uart.println(buff);
    // alarme.agenda(&t3);
    timer.delay(3000000); // e3 = 3 segundos
...
void t4_func(void * arg) {
    ...
    uart.println(buff);
    // alarme.agenda(&t4);
    timer.delay(2000000); // e4 = 2 segundo
...
void setup() {
    char buff[64];
    sprintf(buff, "%ld EDD - Funcionando de acordo (exemplo 1)", TIMER0::micros()/1000);
    uart.println(buff);
    timer.registraObservador(&alarme);

    t0.tempo = 3000000; // d0: 3 segundos
    t0.funcao = t0_func;
    t0.args = (void*) 0;
    t0.prioridade = 1;
    t0.release_time = TIMER0::micros();

    t1.tempo = 10000000; // d1: 10 segundos
    t1.funcao = t1_func;
    t1.args = (void*) 0;
    t1.prioridade = 5;
    t1.release_time = TIMER0::micros();

    t2.tempo = 7000000; // d2: 7 segundos
    t2.funcao = t2_func;
    t2.args = (void*) 0;
    t2.prioridade = 3;
    ...
    t4.tempo = 5000000; // d4: 5 segundos
    t4.funcao = t4_func;
    t4.args = (void*) 0;
    t4.prioridade = 2;
