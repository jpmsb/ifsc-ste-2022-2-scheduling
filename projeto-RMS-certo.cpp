...
void t0_func(void * arg) {
    char buff[32];
    sprintf(buff, "%ld RMS: T0 - inicio", (TIMER0::micros()/1000) - 24);

    uart.println(buff);
    alarme.agenda(&t0);
    timer.delay(2000000);

    sprintf(buff, "%ld RMS: T0 - fim", (TIMER0::micros()/1000) - 24);
    uart.println(buff);
}

void t1_func(void * arg) {
    char buff[32];
    sprintf(buff, "%ld RMS: T1 - inicio", (TIMER0::micros()/1000) - 24);

    uart.println(buff);
    alarme.agenda(&t1);
    timer.delay(2000000);

    sprintf(buff, "%ld RMS: T1 - fim", (TIMER0::micros()/1000) - 24);
    uart.println(buff);
}

void setup() {
    char buff[64];
    sprintf(buff, "%ld RMS - Funcionando de acordo (exemplo 1)", TIMER0::micros()/1000);
    uart.println(buff);
